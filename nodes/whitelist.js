...
module.exports = function (RED) {
        ...
        node.on('input', function (msg) {
            ...
            if (filters.length > 0) {
                if (filterPayload && typeof msg.payload === 'string') {
                    shouldAllow = filters.some(filter => msg.payload.includes(filter));
                }

                if (filterTopic && typeof msg.topic === 'string') {
                    shouldAllow = shouldAllow || filters.some(filter => msg.topic.includes(filter));
                }
            }
    ...
    RED.nodes.registerType("whitelist", WhitelistNode);
}